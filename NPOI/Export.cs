...
namespace NPOIHelper {
    public static class Export {
        ...
        public static MemoryStream RenderToExcelForXlsx(Table table) {
            using (var ms = new MemoryStream()) {
                ...
                if (head != null) {
                    ...
                    for (var i = 0; i < head.Columns.Count; i++) {
                        headerRow.CreateCell(i).SetCellValue(head.Columns[i].Value);
                        CellSetter.SetCellStyle(workbook,headerRow.Cells[i],head.Columns[i].FontColor,true);
                    }
                    //第一个参数表示要冻结的列数；
                    //第二个参数表示要冻结的行数；
                    //第三个参数表示右边区域可见的首列序号；
                ...
                //if (table.BandColumns.Any()) {
                //    foreach (var bandColumn in table.BandColumns) {
                //        CellSetter.SetCellDropdownlist(sheet,bandColumn.Key,bandColumn.Value,true);
                //    }
                //}
        ...
        public static MemoryStream RenderToExcelForXlsx(DataTable table,string sheetName) {
            using (var ms = new MemoryStream()) {
                var workbook = new XSSFWorkbook();
                var sheet = string.IsNullOrWhiteSpace(sheetName) ? workbook.CreateSheet() : workbook.CreateSheet(sheetName);
